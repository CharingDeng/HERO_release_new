...
import json
from utils.motion_process import recover_from_ric  # 需从原项目导入
...
def extract_features(motion_data, joints_num=22):
    ...
    if seq_len < 3:
        ...
        velocity = motion_data[1:] - motion_data[:-1]
    ...
    # 时空特征：关节运动范围
    joints = recover_from_ric(motion_data, joints_num)
    arm_joints = joints[:, [5, 6, 7, 9, 10, 11]]
    leg_joints = joints[:, [12, 13, 14, 15, 16, 17]]
